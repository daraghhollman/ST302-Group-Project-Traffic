...
I, Elise Drivon, had primary responsiblity for sections 1.2, 2.3, and 3.3.
...
I, Catherine Kwagala, had the primary responsability for sections 2.2 and 3.2.
...
## 2.2 Creating a correlation matrix

```{r}
#Select year columns.

years <-2013:2023
year_cols <-intersect(colnames(traffic), as.character(years))

```

```{r}
#All counties + all statistics combined;Keeping only numeric year data.

df_years <- traffic[, year_cols]
df_years <- as.data.frame(
  lapply(df_years, function(x) as.numeric(as.character(x)))
)
...
```{r}
#Create the correlation matrix (years vs years)

#How are years correlated with each other across all counties and statistics?”

cor_matrix <-cor(df_years, use = "pairwise.complete.obs")
print(cor_matrix)
```
## 2.3 Grouping counties into provinces
...
## 3.2 Correlation of accident patterns between years in all counties
...
corrplot(cor_matrix,
         method = "ellipse",#Ellipse shapes
         type = "upper",
         diag = FALSE,
         order = "hclust",#Hierarchical clusteringorder
...
Rows & columns: Years (2013–2023)

Each cell: Correlation of accident patterns between two years

Red: similar patterns

Blue: dissimilar patterns

Ellipses: strength + direction of correlation

Clustering: years with similar accident behaviour grouped together

The correlation plot shows extremely strong positive correlations between years (mostly >0.9) As indicated by the deep red ellipses and values close to 1. Counties with high fatal collisions in one year generally remain high in other years. Hierarchical clustering groups adjacent years together, reflecting similar temporal patterns. These large positive correlations also indicate that, over time,  the spatial distribution of fatal traffic collisions across counties is quite stable. That is, those counties with a large number of fatal collisions in one year  Also tend to have large numbers in subsequent years, whereas low-county counties stay low.  The temporal clustering of successive years with one another suggests smooth changes rather  than abrupt shifts in collision patterns.

The heat map confirms that fatal traffic collisions in Ireland exhibit strong temporal consistency 
across counties from 2013 to 2023. Such persistence indicates that road safety outcomes are determined by long-term structural and regional factors rather than by short-term fluctuations.

## 3.3 Fatal and injury collisions across provinces

```{r, fig.cap="**Figure 3:** Fatal collisions in Ireland from 2013 to 2023"}
...
## 3.4 Fatal and injury collisions of each year across all counties
